...
export function whyNot(claim: {
  ...
  // I-132 C: nothing was actually missed — say so first.
  if (missed.length === 0 && claim.red.length === 0 && waiting.length > 0)
    return `nothing missed — ${list(waiting)} still to come`;
  return parts.join(' · ');
}
