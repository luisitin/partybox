...
export function Tv({ view }: GameTvProps<BingoTvView>): JSX.Element {
  ...
  // I-119 C: the last three seconds tick, quietly.
  useEffect(() => {
    if (!slowGap || view.calledAt === null || view.deadline === null || view.paused) return undefined;
    const ts = [3000, 2000, 1000].map((before) =>
      setTimeout(() => sound.play('tick', { quiet: true }), Math.max(0, view.deadline! - before - Date.now())),
    );
    return () => ts.forEach((t) => clearTimeout(t));
  }, [slowGap, view.calledAt, view.deadline, view.paused, sound]);
