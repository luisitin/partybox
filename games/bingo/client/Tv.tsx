...
import { BALL_LAND_MS, hushCaller, speakCall, LONG_LINE } from './caller';
...
export function Tv({ view }: GameTvProps<BingoTvView>): JSX.Element {
  ...
  useLayoutEffect(() => {
    ...
    speakCall(sound, letter, number, 0, { pack: view.current?.pack ?? 'family', long: (view.current?.call.length ?? 0) > LONG_LINE }); // I-129: and the nickname after it
