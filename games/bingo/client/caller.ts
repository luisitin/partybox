...
/**
 * I-129 C: the short cuts the pack has recorded ("<pack>/<number>"), for lines over LONG_LINE
 * characters — the part before the dash. A long line without one plays in full.
 */
export const SHORT_CUTS: ReadonlySet<string> = new Set<string>(['cheeky/13', 'cheeky/52', 'cheeky/73', 'cheeky/9', 'family/12', 'family/15', 'family/26', 'family/28', 'family/37', 'family/39', 'family/42', 'family/47', 'family/54', 'family/59', 'family/64', 'family/8', 'family/9']);

/** I-129 C: a nickname longer than this on screen is a candidate for its short cut. */
export const LONG_LINE = 34;

/**
 * I-129 A: the nickname's clip — /sfx/nicks/<pack>/<number>.wav (or <number>-short.wav). A number the pack
 * has not recorded says nothing, so a half-finished pack sounds exactly like today.
 */
export function nickClip(pack: 'family' | 'cheeky', number: number, long = false, cutsHave: ReadonlySet<string> = SHORT_CUTS): string {
  const short = long && cutsHave.has(`${pack}/${number}`);
  return `/sfx/nicks/${pack}/${number}${short ? '-short' : ''}.wav`;
}
...
export function speakCall(
  ...
  nick?: { pack: 'family' | 'cheeky'; long: boolean },
): void {
  ...
  if (nick)
    sound.clip(nickClip(nick.pack, number, nick.long), {
