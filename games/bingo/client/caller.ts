...
/**
 * I-129 A: the nickname's clip — /sfx/nicks/<pack>/<number>.wav. A number the pack
 * has not recorded says nothing, so a half-finished pack sounds exactly like today.
 */
export function nickClip(pack: 'family' | 'cheeky', number: number): string {
  return `/sfx/nicks/${pack}/${number}.wav`;
}
...
export function speakCall(
  ...
  if (nick)
    sound.clip(nickClip(nick.pack, number), {
