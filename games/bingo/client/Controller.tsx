...
export function Controller({
  ...
  // I-135 C: a card dealt for the extension comes up the moment it arrives.
  const [cardsSeen, setCardsSeen] = useState(cards?.length ?? 0);
  if ((cards?.length ?? 0) !== cardsSeen) {
    const grew = (cards?.length ?? 0) > cardsSeen;
    setCardsSeen(cards?.length ?? 0);
    if (grew && cards) setUp(cards.length - 1);
  }
