...
function applyDaubAttrs(): void {
  ...
  root.dataset['daub'] = readKey(DAUB_KEY, DAUBS.map((d) => d.id), 'blot');
  const ink = INKS.find((i) => i.id === readKey(INK_KEY, INKS.map((i) => i.id), 'mine'));
...
export function useDaubStyle(): DaubStyle {
  return useSyncExternalStore(subscribeDaub, () => readKey(DAUB_KEY, DAUBS.map((d) => d.id), 'blot'), () => 'blot');
}
export function useInk(): Ink {
  return useSyncExternalStore(subscribeDaub, () => readKey(INK_KEY, INKS.map((i) => i.id), 'mine'), () => 'mine');
}
