...
describe('I-129 the spoken nickname', () => {
  it('starts a breath after the number has been said — never over it', () => {
    const { api, log } = fakeSound();
    speakCall(api, 'O', 69, 0, { pack: 'family', long: false });
  ...
  it('a long line plays its short cut only where the pack has one', () => {
    expect(nickClip('family', 9, true, new Set())).toBe('/sfx/nicks/family/9.wav');
    expect(nickClip('family', 9, true, new Set(['family/9']))).toBe('/sfx/nicks/family/9-short.wav');
    expect(nickClip('family', 9, false, new Set(['family/9']))).toBe('/sfx/nicks/family/9.wav');
  });
});
