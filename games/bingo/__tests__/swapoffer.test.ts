...
describe('I-139 swap offer', () => {
  it('keeps the old card for the choice; keepOld puts it back, takeNew keeps the new one', () => {
    ...
    expect(kept.round.swapped['a'] ?? []).toEqual([]);
    // I-139 C: the swap was not spent — the card can be swapped again
    expect(game.controllerView(kept, 'a').swappable).toContain(0);
