...
// accepts every tap. BINGO! evaluates the card: valid → the round is won (`exits.win`), invalid
// → the check (`exits.check`). The deck running out ends the round via `exits.next`.
...
export function reducePlay(state: State, event: GameEvent<Input>, exits: PlayExits): State {
  if (event.type === 'input') {
    ...
    const claim = evaluate(
      event.playerId,
      round.cards[event.playerId] ?? [],
      round.daubs[event.playerId] ?? [],
      calledNumbers(state),
      round.pattern,
    );
    return claim.valid
