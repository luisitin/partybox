...
describe('I-773: the room judges when the judge is gone', () => {
    ...
    for (const id of s.order) {
      ...
      if (open)
        s = reduce(s, {
          ...
          playerId: id,
          input: { type: 'vote', slot: open.slot },
        });
    }
  ...
  it('a last round the room judged still ends as a judge game (awards, final board)', () => {
    let s = readAll(
      playAll(toAnswer(start({ judge: 'czar', players: 4, timed: true, rounds: 3 }))),
    );
    while (s.round < 3) {
      while (s.phase.id !== 'answer' || s.round < 3) s = timer(s);
      s = readAll(playAll(s));
    }
    const judge = s.czarId as string;
    s = reduce(s, {
    ...
    expect(tv(s).judgeMode).toBe('vote');
    while (s.phase.id !== 'final' && s.phase.id !== 'done') s = timer(s);
    expect(s.settings.judge).toBe('czar');
    expect(tv(s).judgeGone).toBeNull();
  });
});
