...
describe('I-474: awards spread', () => {
  ...
  it('B: a close second without an award takes a later one', () => {
    const close = { ...state, stats: { ...state.stats, fastPlays: { priya: 5, sam: 4 } } } as State;
    expect(who(close)['quick-draw']).toBe('sam');
  });
  it('B: not when the second is well behind', () => {
    const far = { ...state, stats: { ...state.stats, fastPlays: { priya: 10, sam: 4 } } } as State;
    expect(who(far)['quick-draw']).toBe('priya');
  });
});
