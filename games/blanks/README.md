...
| `reveal` | one filled card, big; the ones already read, small; the reader named in czar mode; a quiet bar                                                           | the same card                                               | 1.9 s + 16 ms/char ≤ 4 s (past 8 cards: 1.7 s + 12 ms/char ≤ 3.2 s) → next `reveal` or `judge`; VIP skip → `judge`                                                                                                                                              |
...
Three decks, `mild.json` (clean, 472 black / 1395 white), `crude.json` (adult, 329 / 1024) and `wild.json`
(explicit, 379 / 1425) — 5 024 cards, about one black card in twelve a Pick 2 or Pick 3: `{ id, name, rating, black: [{ id, text, pick, draw }], white: [{ id, text }] }`,
