...
function JudgeGrid({ view }: Props): JSX.Element {
  ...
  const longest = Math.max(
    ...
    ...view.cards.map(
      (c) =>
        fillText(
          view.black?.pick === 1 && count <= 8
            ? anchorOf(view.black.text)
            : (view.black?.text ?? ''),
          c.whites,
        ).length,
    ),
    ...
    <>
      ...
      {answersOnly && view.black ? (
        <FilledCard
          ...
          size="grid"
          className={styles.judgeQuestion}
        />
      ) : null}
      <ul
        ref={ref}
        ...
        {view.cards.map((c, i) => (
          <li key={c.slot} style={{ animationDelay: `calc(${i} * 150ms)` }}>
            ...
            >
              ...
              <FilledCard
                text={
                  // past eight the answers alone still win (I-156 C: one page for a big room)
                  count > 8
                ...
                className={
                  answersOnly && count <= 8 && anchor !== '____' ? styles.anchored : undefined
                }
