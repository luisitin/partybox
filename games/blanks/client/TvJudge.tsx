...
function Progress({ view }: Props): JSX.Element {
  ...
  if (n === 0 && gone)
    return (
      <>
        {gone.why === 'kicked'
          ? L('{name} was removed — everyone votes this one · 0 / {expected}', { name: gone.name, expected: m })
          : gone.why === 'left'
            ? L('{name} left — everyone votes this one · 0 / {expected}', { name: gone.name, expected: m })
            : L('{name} dropped — everyone votes this one · 0 / {expected}', { name: gone.name, expected: m })}
      </>
    );
