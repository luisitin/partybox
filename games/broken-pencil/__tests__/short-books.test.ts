...
import { turnPage } from '../server/phases/show';
...
const state = (bot: boolean): State =>
  ({
    ...
    books: [
      { ownerId: 'b', pages: [{ kind: 'word', text: 'cat' }, { kind: 'guess', text: 'dog' }, { kind: 'guess', text: 'hog' }, { kind: 'guess', text: 'log' }] },
      { ownerId: 'p', pages: [{ kind: 'word', text: 'sun' }, { kind: 'guess', text: 'bun' }] },
    ],
