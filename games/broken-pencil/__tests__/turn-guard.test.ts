...
import { reduceShow, showPage } from '../server/phases/show';
...
describe('I-491: the presenter turns on purpose', () => {
  ...
  it('back shows the previous page of the same book', () => {
    const s = showPage(state(), 0, 2, 5000);
    const b = reduceShow(s, { type: 'input', now: 9000, playerId: 'a', input: { type: 'back' } }, (x) => x);
    expect(b.showing?.page).toBe(1);
  });
});
