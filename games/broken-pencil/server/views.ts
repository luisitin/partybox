...
export interface PencilControllerView extends ControllerView, Common {
  ...
  showing: null | {
    ...
    lastPage: boolean;
    lastBook: boolean;
    /** I-228 B: the presenter's next page (face down on their phone); null for everyone else. */
    next: PageView | null;
    ...
    /** The verdict on stage (a last page), so the VIP's phone can call "close enough". */
    verdict: 'intact' | 'broken' | null;
  };
  summary: BookSummary[] | null;
    ...
    showing:
      state.phase.id === 'show' && showing && shownBook
        ? {
            ...
            next: (() => {
              if (shownBook.ownerId !== playerId) return null;
              const p = shownBook.pages[showing.page + 1];
