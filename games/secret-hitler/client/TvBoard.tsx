// The TV's boards (§8.1, plain for M1): the Liberal track (5 slots) and the Fascist track (6
// slots, each labelled with its power) side by side, the HITLER ZONE line after Fascist slot 3,
// and the middle band — the election tracker, deck and discard counts, the veto and the next
// Fascist slot. (M2 stacks the tracks and adds the Parliament Record beside them.)
          ...
          <div className={styles.slots}>
            {[0, 1, 2, 3, 4].map((i) => (
              <div key={i} className={styles.slot}>
                {i < view.board.L ? <PolicyCard party="L" /> : <CardBack />}
                <span className={styles.slotLabel}>{i === 4 ? L('Victory') : ''}</span>
              </div>
            ))}
          </div>
        </div>
        <div className={styles.track}>
          ...
          <div className={styles.slots}>
            {view.slots.map((power, i) => (
              <div key={i} className={styles.slot} data-zone={i === 3 || undefined}>
                {i === 3 ? (
                  <span className={styles.zone} data-on={zone || undefined}>
                    {L('Hitler zone →')}
                  </span>
                ) : null}
                {i < view.board.F ? <PolicyCard party="F" /> : <CardBack />}
                <span className={styles.slotLabel}>
                  {i === 5 ? L('Victory') : power ? powerName(L, power) : ''}
                </span>
              </div>
            ))}
          </div>
