...
if (values.md) {
  ...
  const lines: string[] = [
    ...
    '',
    `Generated by \`pnpm blanks-fit-report --decks ${preset} --md\`. Kinds: what a prompt's blank wants / what a white card serves (server/fit.ts); ★ = tier (1 filler, 2 good, 3 great; server/content and the deck JSON).`,
    '',
    '## Prompts by kind',
  ];
