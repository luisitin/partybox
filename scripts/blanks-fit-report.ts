...
const transcriptRounds = Number(values.rounds);
...
for (let r = 0; r < runs; r += 1) {
  ...
  for (const ev of result.events as GameEvent<Input>[]) {
    ...
    if (
      ...
      before.phase.id !== 'result' &&
      transcript.length < transcriptRounds
    ) {
      const black = blackCard(state.blackId);
      const votesFor = new Map<number, number>();
      for (const v of Object.values(state.votes)) votesFor.set(v, (votesFor.get(v) ?? 0) + 1);
      const who = (id: string): string => {
        const p = result.init.players.find((x) => x.id === id);
        return id === 'rando' ? 'Rando' : p?.bot ? `bot ${p.name}` : (p?.name ?? id);
      };
      transcript.push(
        [
          `### Round ${state.round} · ${black.text} _(${slotsOf(black).join('+')}, ${'★'.repeat(blackTier(state.blackId ?? ''))})_`,
          '',
          ...state.slots.map((id, i) => {
            const cards = state.submissions[id] ?? [];
            const won = state.winners.includes(id) ? ' **← wins**' : '';
            return `- ${String.fromCharCode(65 + i)}. ${fillText(black.text, cards.map(whiteText))} — ${who(id)}, ${cards.map((c) => '★'.repeat(tiers.get(c) ?? 2)).join(' ')}, ${votesFor.get(i) ?? 0} vote(s)${won}`;
          }),
          '',
...
if (values.md) {
  ...
  const lines: string[] = [
    ...
    '',
    '## How the cards are dealt and played',
    '',
    '- **Kinds.** A prompt wants a _thing_ (most), a _doing_ ("What did the sex robot refuse to do?"), a _person_ ("Who…?") or a _name_ (a quoted blank, a safe word, a title, a line someone says; a _word_ blank wants one or two words; a _line_ — a title, a chapter, something said — takes a whole card). A white card is a thing, a doing (a gerund or an event), a person, and — when short — a name. Read off the text by `server/fit.ts`; `slot` / `slots` / `serves` in the deck JSON override it.',
    '- **Hands.** Ten cards, always: 2 things, 3 doings, 2 people, 2 names, one of them a one-or-two-word card for the safe-word blanks; 5 great cards, 2 of them amazing; at most 2 filler; no four cards on one subject (`server/deal.ts`). Once the prompt is known, the hand leads with the cards that read best in its blank, so the first screenful on the phone is the four best answers.',
    '- **Prompts.** The black deck leads with the great prompts and keeps the filler for the back, its Pick 2 / Pick 3 cards spread out, never two in a row; the czar bot picks the best-rated of its three.',
    "- **Bots.** A bot plays the card that reads best blank by blank — fit for the kind, the card's tier in proportion to the fit, a nudge for a card on the prompt's subject in other words and against one that echoes its word, the card's own punch (a twist after a comma, a specific), one or two words for a word blank — and votes the same way (`server/bot.ts`, `server/topics.ts`). Rando plays great cards.",
    '- **Reading it.** `--sample N` prints filled prompts, `--hands N` hands as a phone shows them, `--transcript file` rounds as the room hears them (reports/design/blanks-transcript.md).',
    '',
    '## Prompts by kind',
  ];
...
if (values.transcript && transcript.length > 0) {
  const head = [
    `# Blanks — ${transcript.length} rounds as the room hears them (${preset}, ${players} players, seed ${seed0})`,
    '',
  ...
  writeFileSync(values.transcript, [...head, ...transcript].join(String.fromCharCode(10)), 'utf8');
  console.log(`transcript → ${values.transcript}`);
}
