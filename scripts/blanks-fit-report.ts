...
if (values.md) {
  ...
  const lines: string[] = [
    ...
    '',
    '- **Kinds.** A prompt wants a _thing_ (most), a _doing_ ("What did the sex robot refuse to do?"), a _person_ ("Who…?") or a _name_ (a quoted blank, a safe word, a title, a line someone says; a _word_ blank wants one or two words). A white card is a thing, a doing (a gerund or an event), a person, and — when short — a name. Read off the text by `server/fit.ts`; `slot` / `slots` / `serves` in the deck JSON override it.',
