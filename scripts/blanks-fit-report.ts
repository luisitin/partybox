...
if (values.md) {
  ...
  const lines: string[] = [
    ...
    '- **Hands.** Ten cards, always: 2 things, 3 doings, 2 people, 2 names, one of them a one-or-two-word card for the safe-word blanks; 5 great cards, 3 of them amazing; at most 2 filler; no four cards on one subject (`server/deal.ts`). Once the prompt is known, the hand leads with the cards that read best in its blank, so the first screenful on the phone is the four best answers.',
