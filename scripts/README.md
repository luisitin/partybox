...
| `resolve-loop-log.ts` | `pnpm resolve-loop-log`       | resolve a `reports/design/loop-log.md` merge conflict: main's rows once each + HEAD's new rows; a HEAD row on a number main already used by the OTHER game is renumbered past the top, never dropped |
