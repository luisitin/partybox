...
| p10–p11 + stage                       | merged 6a4752a5; then the shell's start stage (ADR-053) landed: Fake-Out's own intro doubled it; hive-rank [32d48b]: ES SE lie scrolled 64 px                                                                                                                                                                                                                                                                             | intro phase deleted (state, input, views, screens, Countdown, strings, CSS); init starts on question 1, which now asks for the fixed lines; the pause-resume re-check covers lie/pick (closeIfDone after every VIP action; test); the English note moves to the question screens (TV + phone), the lie screen keeps a short "· en inglés" on its kicker, and the ES rules step says "(en inglés)"; stage sweep for fake-out: 32 stills, 0 problems; p11 TV worst frame 183 ms                                                              |
| style1–2 (owner play-test 2026-09-25) | owner: the fact card didn't match Blanks/Wisecrack, the read-along made no sense with audio off; the grid should hold only the truth + players' lies                                                                                                                                                                                                                                                                      | the fact is Blanks' black card (ink, hairline, left-aligned); the blank is a paper line, the truth drops in as a cream paper mark; answers (grid, spotlight, deck) are white paper cards; read-along removed everywhere; MIN_OPTIONS 5 → 3 (house lies only when too few entries to choose); lie/pick/scores cut in quickly (spy-grid's cross-fade nit)                                                                                                                                                                                    |
| p13–p14                               | session-c [9fee27]: TV still 5–7 s with the read-along gone; 200 % lie card too tall; lowercase truth; navy unpicked chips; scores line off baseline; lies pill on the host bar                                                                                                                                                                                                                                           | the question card, the completed fact, the unpicked strip and the score reasons breathe (audio-independent, still under reduced motion); lie card capped by phone width; truth uppercase; unpicked = paper; .how row aligned; the lie tray gives way before the host bar. p14 TV: 1 still of 1.5 s (scores, now breathing), worst frame 117 ms; the one 'hard cut' is the harness's budget skip                                                                                                                                            |
| p15 + style4–5                        | session-c [ba4fbb] + reviewer [f9d6d3]: the per-lie spotlight held still (2.8 s); its faces flickered; 200 % lie box too short (text scrolled, counter on it); reason chips looked boxed; 200 % Suggest pill squashed; 200 % pick broke 'Gingerbrea / d'                                                                                                                                                                  | spotlight wrapper breathes; Spotlight keyed by step only (a late reading's re-time no longer remounts it); lie box 3 lines + field-sizing, counter below; rows/chips breathe transform-only (no outline); lie pieces flex-shrink 0; pick text capped by phone width. p15 TV: 0 dead spans, worst frame 117 ms (the 2 'cuts' are the harness's budget skip)                                                                                                                                                                                 |
| style6                                | session-c [592e6f]: 'picked by' showed alone for ~0.4 s at a hand-off; the TV scores' reason row wrapped under the host bar; the writer's name clipped under 'written by'; reviewer nit: the stamp covered a long answer's last letter                                                                                                                                                                                    | the caption pops in with the first face; big faces only under 'picked by' (written-by back to normal); scores reserve the host bar's height and show 4 reasons; a stamped card pads its right side                                                                                                                                                                                                                                                                                                                                         |
| p16                                   | session-c [90140d]: 'Next: the Final Fake-Out' ran under the host bar; written-by names overlapped the deck row; the TRUTH ✓ stamp covered a letter                                                                                                                                                                                                                                                                       | the final teaser sits above the board; written-by names sit beside their faces; the stamp room is sized to TRUTH ✓                                                                                                                                                                                                                                                                                                                                                                                                                         |

The score screen keeps a reason row for every player who scored. At the 11–12 player maximum, the
standings use three columns so the full reason list has room above the host bar ([4e7487], [798659]).
