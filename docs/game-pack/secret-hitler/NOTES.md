...
## The start is the shell's (ADR-053, main bc3f879c)

The shell's start stage shows the manifest's three `howToPlay` steps, takes everyone's READY and
counts 3 · 2 · 1. The game's own briefing, READY stamps and count-in are gone. Seating is game
content only: each player reads their secret dossier and taps "I've read it". The TV shows the front
page, "n of m have read their dossier" and a ✓ seal per seat. Round 1 begins as soon as every
connected seat has read theirs (bots at once; a drop doesn't block). The VIP's "Start now" begins
round 1. The group-standard hidden net (hub e67ec9) ends idle tables: at 3 min with nobody tapped it
starts; if someone has, it re-arms up to 10 min in all. A pause holds any leave or drop; the phase
settles at resume (`recheckOnResume`).
