...
*Batch 6 of 7 · read Part 00 first · game 9 of the pack.*
...
2. **The guesses.** Sun's players talk it over and tap words on their phones. (Remote players without a call point silently instead.) When most of the team points at the same word, it flips:
   ...
   - ● **Their agent:** the turn ends, and we've just found one of *theirs* for them.
...
const SpyInput = z.discriminatedUnion('type', [
  ...
  z.object({ type: z.literal('clue'), word: z.string().min(1).max(40), number: z.number().int().min(1).max(9) }),
  z.object({ type: z.literal('point'), target: z.union([z.number().int().min(0).max(24), z.literal('end')]) }),
  z.object({ type: z.literal('unpoint') }),
  z.object({ type: z.literal('react'), card: z.number().int().min(0).max(24), emoji: z.enum(['👍', '👎', '🤔']) }),
]);
```
...
type SpyState = {
  ...
  history: { team: Team; spymaster: PlayerId; word: string; number: number; flips: { card: number; kind: Kind; first: PlayerId | null }[] }[];
  stats: Record<PlayerId, { clues: number; agentsFromClues: number; bestClue: number; sharp: number; trap: number }>;
...
```json
{ "id": "shark", "word": "SHARK", "themes": ["ocean", "danger", "fish"], "hints": ["jaws", "fin"], "family": ["shark"] }
```
...
```json
{
  ...
  "members": ["shark", "ship", "wave", "whale", "anchor", "pirate", "coral", "island", "diver", "submarine"]
}
```
