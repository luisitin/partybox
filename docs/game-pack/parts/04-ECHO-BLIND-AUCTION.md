...
*Batch 5 of 7 · read Part 00 first · games 7 and 8 of the pack.*
...
4. **Result.**
   ...
   - **Wrong:** the word is discarded, *and so is the next word* in the deck. A wild guess costs double.
...
type EchoState = {
  ...
  w: {
    ...
    dontKnow: PlayerId[]; swaps: number;
...
```json
{
  ...
  "family": ["tele", "scope"],
  "clues": ["stars", "space", "lens", "astronomer", "Galileo", "zoom", "observatory", "planets", "moon", "sky"]
}
```
...
type AuctionState = {
  ...
  l: {
    ...
    stage: 0 | 1 | 2; openedAt: number;
    winner: PlayerId | null; price: number;
    effect: { kind: string; amount: number; other?: PlayerId } | null;   // filled at flip
  };
  coins: Record<PlayerId, number>;
  stats: Record<PlayerId, { biggestBid: number; bestProfit: number; thief: number; trapped: number; spent: number }>;
  speechMs: Record<string, number>;
};
```
