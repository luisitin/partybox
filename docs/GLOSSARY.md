...
| **Effect**       | Output of the engine besides the new room state: `push`, `toast`, `kicked`, `error`, `log`. Only the server interprets effects; timing comes from `nextWakeAt` + `tick` (ADR-022). |
