...
## ADR-034 — The web build hosts the room in a browser tab, over WebRTC

**Context.** The owner wants the same games playable by people who are not in the same house, from a GitHub Pages URL. Pages serves static files and nothing else: there is no process to run Fastify, no Socket.IO, no place to keep a room. The LAN app must keep working exactly as it does (ADR-012: no internet at runtime), and future work on games and shells must land in both without being written twice.
**Decision.** (1) The room host moves out of `packages/server` into `packages/host` and stops being Node-specific: Web Crypto instead of `node:crypto`, portable timer handles, I/O only through `Transport` and `Clock`. `packages/server` now wires it to Socket.IO; `packages/web` wires it to WebRTC. One engine, one host loop, two wires. (2) On the web the **first player's browser tab is the server**: it runs `@partybox/host` with the real engine and the real game reducers, and every other player opens a WebRTC data channel to it. Room codes double as the address — the host claims `partybox-v1-<CODE>` on PeerJS's public broker, which is used for signalling only; a code another party holds is swapped for the next one the engine mints. (3) **There is no TV.** Each device renders both surfaces of the same room: the real `TvApp` in a 16:9 strip at the top (its music, beds, cues and Bingo caller now come out of that phone) and the real `ControllerApp` under it. Both stores are transport-agnostic (`NetTransport`), so the web build reuses the client's screens rather than copying them. (4) The stage's host controls run as the sending player under the ordinary VIP rules — nobody inherits the TV's no-VIP-check powers (ADR-031), because on the web the TV is everybody's phone. (5) `/api/info` becomes swappable (`setInfoProvider`): on Pages the join URL, the room code and the QR are computed in the browser.
**Consequences.** Games are untouched and need no per-build work: a new game reaches both builds through `pnpm gen-registry`. The host tab is a single point of failure — closing it ends the party, and it is the one thing the LAN version does not have. WebRTC is peer-to-peer, so players learn each other's IP addresses, and a symmetric NAT needs a TURN server (`VITE_TURN_*`). A backgrounded host tab throttles timers, so deadlines can fire a second late. `WEB_DEPLOY.md` is the merge guide between the two builds.
