...
**Decision.** `room.contentLang` ('en' | 'es', `CONTENT_LANGS`). Effective value (`engine/content-lang.ts`): the explicit choice if someone made one (the VIP's switch in the ★ menu, "Cards and voice in Spanish"; the TV's language switch also sets it); else the VIP phone's language (phones send `join.lang`); else 'en'. It is on the room snapshot (`RoomSnapshot.contentLang`, so a client-rendered deck picks its pack without a round trip) and in `InitContext.contentLang`, fixed for the game at its start; a switch mid-game is for the next game. The chosen-game screen names it ("🗣 Cards in Spanish"). The host's reader adds Kokoro's Spanish voices `dora` / `alex` / `santa` spoken as `es-419` (Latin American seseo; `es` would be Castilian). The TV's own UI keeps its language (no forced follow: a Spanish TV must not flip to English before a Spanish VIP joins).
