...
**Decision.** `GameResults` gains two optional fields, set by the game's server (the recap reads them too): `outcome` — `{ kind: 'coop', won }` or `{ kind: 'teams', winner: teamId | null, teams: { id, name, mark?, members }[] }` (`null` = a draw; `mark` ▲ / ● so a team never rests on colour alone) — and `headline`, the results line in English, translated through the game's strings like awards. The shell's line is the headline, else "Mission complete!" / "Mission failed" / "Sun wins! ▲" (the mark closes the line, never mid-sentence: "¡Gana Sol! ▲") / "A draw!"; a win or completed mission gets the full confetti and the cheer (the winning team's faces beside the line), a loss or draw the tie chord and no confetti. `winnerIds` may be empty only for a co-op game the players lost (contract suite and sim invariant).
**Consequences.** Games without `outcome` are unchanged. Co-op and team games set it and drop their crown-everyone fallbacks. Follow-up (2026-09-25): a team may carry `color` (the game's own CSS colour or token); for a team game the TV and phone boards are grouped by team — each group headed by its mark + name in its colour, the winners first and outlined with a 🏆 Winners tag, no individual trophies or ranks — the headline takes the winner's colour, and a phone shows "Your team won!" / "Your team lost this one" in place of its individual place (a tap brings its own team's group into view). A player in no team's members (one who left) is listed in a last "No team" group, never dropped.
