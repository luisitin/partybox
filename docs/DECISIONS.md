...
## ADR-047 — Where everyone is: a room mode and a per-phone "I can see the TV", fixed for the game at its start

**Context.** Game pack Part 00 §3 (the owner, 2026-09-24; rulings 1, 13, 14): some parties share one TV, others have friends playing from home who cannot see or hear it, on a call or not. Games must know which, to switch talk timers, live bidding or team huddles on or off — but `init` took only players, settings, seed and time (brief rule 13), a reserved `__presence` setting cannot work (`coerceSettings` drops unknown keys, `SettingValue` cannot hold an object; audit #11), and the phone-only fork (ADR-041) was room-wide, so a remote player in a TV room got "look at the TV" (audit #13).
**Decision.** The room gains `presenceMode` — `together` (default), `remote-voice`, `remote-text` — a VIP action `setPresenceMode` beside Phone only, rejected mid-game, carried in the snapshot. Each person gains `canSeeTv`: the phone's 🎨 toggle (remembered on the phone for that room and that night only, so last week's answer never overrides tonight's couch), or, until it says, the host's guess from its address — RFC 1918, link-local, ULA and plain loopback see the TV; a loopback peer carrying a forwarded header (the Cloudflare tunnel, I-646) and everything else (100.64/10 and Tailscale's fd7a:115c:a1e0::/48 inside ULA too) do not; bots always see it. `InitContext.presence = { mode, phoneOnly }` and `PlayerInfo.canSeeTv` are optional additions, fixed at start like the seed (the brief's rule 13 is amended: "presence fixed at start is not carry-over"; ruling 1); a game uses them only to switch features, never to branch view content. The engine stamps `phoneOnly` per phone — `room.phoneOnly || !canSeeTv` — so every game's existing stage fork and `PhoneStage` reach a remote phone in a TV room, and a mid-game flip moves only that phone; `useCanSeeTv()` keeps the copy honest. Remote phones get readings, TV cues and music by default, with the 🎨 opt-outs (ruling 14). The "Maya can't see the TV — are you on a call?" prompt is derived from the snapshot and shown on the VIP's phone and the TV host bar, between games, never in phone-only rooms. Choosing a `voice-if-remote` game in a `remote-text` room, or a `same-room` game with anyone away from the TV, shows the manifest's notice first.
**Consequences.** Games that ignore presence behave as before; the five existing games gain remote play through the per-phone stamp alone. Determinism holds (presence is a seeded input); the sim and contract suites run the three modes × phone-only × a mixed roster. The harness forces a phone remote with `?canSeeTv=0`; the recorder saves each game's presence and who was remote in `session.json`. The runner puts `canSeeTv` on every `PlayerInfo`, so every game's state (and its hash) gains the field: replays stay deterministic, but recorded fixtures change once. `PlayerPublic.canSeeTv` tells the whole room who joined from off the LAN — intended, the prompt needs it. The shell pieces: the 🎨 sheet's "This phone" group (`ThisPhone`: I can see the TV, TV sounds, vibration), the ★ menu's `PresenceSwitch`, `PresencePrompt` and the TV's chip, `PresenceNotice` on the VIP's chosen screen, `useCanSeeTv()` in the SDK.
...
## ADR-052 — A co-op or team game says how it ended; the shell stops calling it a tie

**Context.** The contract's results only name winners (`winnerIds`, never empty). Echo and Tune In's co-op, Spy Grid's failed mission and Tune In's teams (2026-09-24, #plans f791dc) had to crown everyone, so the shell read "It's a tie!" over a failed run and "Lu, Sam & the bot tie!" over Sun's 11–8 win.
**Decision.** `GameResults` gains two optional fields, set by the game's server (the recap reads them too): `outcome` — `{ kind: 'coop', won }` or `{ kind: 'teams', winner: teamId | null, teams: { id, name, mark?, members }[] }` (`null` = a draw; `mark` ▲ / ● so a team never rests on colour alone) — and `headline`, the results line in English, translated through the game's strings like awards. The shell's line is the headline, else "Mission complete!" / "Mission failed" / "▲ Sun wins!" / "A draw!"; a win or completed mission gets the full confetti and the cheer (the winning team's faces beside the line), a loss or draw the tie chord and no confetti. `winnerIds` may be empty only for a co-op game the players lost (contract suite and sim invariant).
**Consequences.** Games without `outcome` are unchanged. Co-op and team games set it and drop their crown-everyone fallbacks.

## ADR-053 — Every game starts through one shell stage: rules, everyone's READY, 3·2·1

**Context.** The owner's pacing rule [cc45f4] (2026-09-24): after Start, a rules screen; every connected player taps READY (bots don't wait, dropped phones don't block); then 3·2·1. Nothing may move on before a slow reader could read it. Four game branches were building this game-side, each with its own look, its own idle timeout for the contract suite, and its own drift during a pause.
**Decision.** The shell owns it (`engine/start-stage.ts`). The VIP's Start (and Play again) opens `room.starting = { gameId, settings, seed, ready, countdownAt }` while `status` stays `selecting`, so every screen keyed on the status keeps working and `screenKey` gives the stage its own cross-fade. People tap READY (the `ready` message); the stage waits for every connected human (a "spectator" between games plays the next one, so they read too), never for bots or dropped phones; a phone back before the count owes its READY again. No timer anywhere: when nobody is left to wait for, `countdownAt = now + 0.4 s`, and the host's tick at `countdownAt + 3 s` starts the game with the settings and seed Start fixed (`canStart` is checked again; a room that no longer suits the game goes back with the reason). The VIP (or the TV) has ‹ Back (the picker), Start now (the count at once) and, during the count, Wait (`pause`): the count stops and the rules stay up until Start now counts from 3 again, so nothing is frozen and shifted and nothing can drift. Settings changes wait for the picker. Nothing counts for an empty room: if every person's phone drops (the room has people, none connected), the count stops and the rules wait; a Wait ends when the VIP who said it drops. On a phone, READY is offered once the last step has been on screen: while it is below the fold (an SE, 200 % text) the button reads "↓ Read all 3 steps" and a tap scrolls to it. Screens: the phone shows the game, all three how-to-play steps at full strength and a sticky READY with who the room still waits for; the TV shows the same rules large and a row of faces that light (✓ and a ring, never colour alone) per READY. The 3·2·1 is one component on both, each number derived from `countdownAt` so they land on the same frame; the TV plays a cue per number, a phone only when it carries the room's sound (phone only, or it can't see the TV); reduced motion changes the number without the zoom. `POST /api/dev/start` stays instant unless `stage: true`; `startNow` outside a stage is the instant start the tests use.
**Consequences.** Games drop their own rules/READY/count and their idle nets; what stays is game content that must follow the start (Bingo's card pick, Nightfall's hold-your-card, Spy Grid's team pick, Secret Hitler's seating, Tune In's team roster). Design tools that press Start in the UI tap READY on each phone (`capture-picker`); `sweep-stage.ts` films the stage on every phone size, in both languages, with the TV.

## ADR-054 — A room has one content language; each device keeps its own UI language

**Context.** The owner (2026-09-25): a Spanish room should get Spanish content, not only a Spanish UI (Session C's full-Spanish plan [78a99f]). Decks, bot lines, the reader's voice and the answer matcher are shared by the whole room, so one game must run its content in one language; each phone's and the TV's UI language already stay on the device (setLang).
**Decision.** `room.contentLang` ('en' | 'es', `CONTENT_LANGS`). Effective value (`engine/content-lang.ts`): the explicit choice if someone made one (the VIP's switch in the ★ menu, "Cards and voice in Spanish"; the TV's language switch also sets it); else the VIP phone's language (phones send `join.lang`, and `lang` whenever it changes); else 'en'. A game plays in the room's language only if its manifest ships it (`contentLangs`, absent = English); otherwise in its own, and the chosen-game chip says so ("Cards in English (no Spanish version yet)"). It is on the room snapshot (`RoomSnapshot.contentLang`: the NEXT game's, shown on the chosen-game screen) and in `InitContext.contentLang`, fixed for the game at its start; the running game's own language rides on every game view (`PushedView.contentLang`), so a client-rendered deck reads that and never flips if someone switches mid-game (Session C's review); a switch mid-game is for the next game. The chosen-game screen names it ("🗣 Cards in Spanish"). The host's reader adds Kokoro's Spanish voices `dora` / `alex` / `santa` spoken as `es-419` (Latin American seseo; `es` would be Castilian). The TV's own UI keeps its language (no forced follow: a Spanish TV must not flip to English before a Spanish VIP joins).
**Consequences.** A game with a Spanish pack loads it by `ctx.contentLang`, passes that language to the SDK matcher and speech (`toSpeakable` already keeps accents in 'es'), and defaults its reader to a Spanish voice; a game without one ignores it and keeps the "en inglés" marker. Session C translates the packs as data files; the owning sessions wire them.

## ADR-055 — AGENTS.md is the one rulebook; CLAUDE.md only imports it

**Context.** The owner is adding OpenAI Codex agents beside the Claude sessions (2026-09-25, hub #plans [cf67dd]). Codex reads `AGENTS.md`: a global one, then one file per folder from the git root down to the folder it started in, 32 KiB in all. Claude Code reads `CLAUDE.md`, and while any `CLAUDE.md` sits on the path it skips `AGENTS.md` unless a `CLAUDE.md` imports it. Two copies of the same rules would drift, and a symlink breaks on Windows clones (Git checks one out as a one-line text file unless `core.symlinks` is on).
**Decision.** A folder's rules live in its `AGENTS.md`. The `CLAUDE.md` beside it is a 3-line pointer: `@AGENTS.md` (Claude Code's import) and one sentence for anyone who opens the file directly. The line caps move with the rules (root ≤ 120, package or game ≤ 30). `pnpm check-drift` fails a `CLAUDE.md` that holds rules next to an `AGENTS.md`, a pointer with no `AGENTS.md`, and a repo root without `AGENTS.md`. A package or game folder that still has only a `CLAUDE.md` passes under the old cap, so branches cut before the switch stay green until they convert (`git mv CLAUDE.md AGENTS.md`, then the pointer). `pnpm new-game` writes the new game's rules to `AGENTS.md`. `docs/CODEX_GUIDE.md` onboards any agent with no memory of the project, carrying what used to live only in Claude session notes, in tool-agnostic words.
**Consequences.** Edit `AGENTS.md`, never `CLAUDE.md`. A Codex agent started inside the repo never sees `C:/dev/AGENTS.md` (the machine's hub rules, outside git), so the root `AGENTS.md` sends every agent there first. `REQUIRED_GAME_FILES` still lists `CLAUDE.md`, since every game keeps one. Once the in-flight game branches have converted, `AGENTS.md` can join that list and the legacy branch of the check can go.
