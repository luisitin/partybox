...
export interface GameDefinition<S extends GameStateBase, I> {
  ...
  tvView(state: S): TvView; // JSON; identical for every TV
  controllerView(state: S, playerId: string): ControllerView; // JSON; per player
...
```ts
export function reduce(state: S, event: GameEvent<I>): S {
  if (event.type === 'player') return setConnected(state, event); // shared helper
  if (event.type === 'vip') return applyVip(state, event, advance); // shared helper: pause/resume/skip/end
  if (event.type === 'timer' && !isTimerFor(state, event)) return state; // stale timer → ignore
  switch (
    state.phase.id // one file per phase
  ) {
    case 'answer':
      return answerPhase.reduce(state, event);
    case 'reveal':
      return revealPhase.reduce(state, event);
    default:
      return state;
  }
}
...
| 5   | Views never throw for any `(state, playerId)` — spectators, disconnected, unknown ids — and never leak hidden info.                           | fuzz + each game declares `hidden` fields in its contract-test config                     |
...
3. **`answer` phase**: `input` stores the trimmed answer for a _playing_ player (spectators ignored);
   when every connected player has answered **or** the `timer` fires → `enterReveal(state, now)`.
