...
| `vip`   | `{ action, ... }`                                   | `selectGame { gameId }` (`null`: the game list with nothing chosen — ADR-051), `highlight { gameId }` (the VIP's open About, mirrored on the TV; `null` closes it — ADR-051), `updateSettings { settings }`, `start`, `skip`, `pause`, `resume`, `end`, `kick { playerId }`, `transferVip { playerId }`, `lock`, `unlock`, `playAgain`, `toLobby`, `setRecording { on }` (ADR-035; any time but mid-game), `setMusicOnPhones { on }` (ADR-040), `setPhoneOnly { on }` (ADR-041), `reclaimVip` (I-347: only from the `formerVip` whose role passed on mid-game; until the next game starts or the role moves again). Ignored (and counted for rate limiting) when the sender is not VIP. |
