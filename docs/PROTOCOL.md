...
| `vip`      | `{ action, ... }`                                   | `selectGame { gameId }` (`null`: the game list with nothing chosen — ADR-051), `highlight { gameId }` (the VIP's open About, mirrored on the TV; `null` closes it — ADR-051), `updateSettings { settings }`, `start` (ADR-053: opens the start stage), `startNow` (in the stage: the 3·2·1 at once; outside one: the game starts at once), `back` (the stage → the picker), `skip`, `pause` (in the stage: Wait, the count stops), `resume` (in the stage: as `startNow`), `end`, `kick { playerId }`, `transferVip { playerId }`, `lock`, `unlock`, `playAgain`, `toLobby`, `setRecording { on }` (ADR-035; any time but mid-game), `setMusicOnPhones { on }` (ADR-040), `setPhoneOnly { on }` (ADR-041), `setPresenceMode { mode }` (ADR-047: `together` | `remote-voice` | `remote-text`; any time but mid-game), `reclaimVip` (I-347: only from the `formerVip` whose role passed on mid-game; until the next game starts or the role moves again). Ignored (and counted for rate limiting) when the sender is not VIP. |
