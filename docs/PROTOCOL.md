...
| Event      | Payload                                                   | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| ---------- | --------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `join`     | `{ roomCode?, name, avatarId, token?, canSeeTv?, lang? }` | `token` re-authenticates a previous player (reconnect). Without a token, a name that belongs to a currently DISCONNECTED player resumes that player (ADR-029). Without a code and with exactly one open room, joins that room.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `input`    | `{ seq, input }`                                          | `input` is validated against the current game's `inputSchema`; `seq` is a per-client counter used to ack/dedupe.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `vip`      | `{ action, ... }`                                         | `selectGame { gameId }` (`null`: the game list with nothing chosen — ADR-051), `highlight { gameId }` (the VIP's open About, mirrored on the TV; `null` closes it — ADR-051), `updateSettings { settings }`, `start` (ADR-053: opens the start stage), `startNow` (in the stage: the 3·2·1 at once; outside one: the game starts at once), `back` (the stage → the picker), `skip`, `pause` (in the stage: Wait, the count stops), `resume` (in the stage: as `startNow`), `end`, `kick { playerId }`, `transferVip { playerId }`, `lock`, `unlock`, `playAgain`, `toLobby`, `setRecording { on }` (ADR-035; any time but mid-game), `setMusicOnPhones { on }` (ADR-040), `setPhoneOnly { on }` (ADR-041), `setPresenceMode { mode }` (ADR-047: `together` | `remote-voice` | `remote-text`; any time but mid-game), `setContentLang { lang }` (ADR-054: `en` | `es`; any time, for the next game — the TV's language switch sends it too), `reclaimVip` (I-347: only from the `formerVip` whose role passed on mid-game; until the next game starts or the role moves again). Ignored (and counted for rate limiting) when the sender is not VIP. |
...
| `lang`     | `{ lang: 'en' \| 'es' }`                                  | ADR-054: this phone's language changed (🎨). The VIP's is the room's content language until someone chooses one. Costs 2 rate-limit tokens.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `ready`    | `{}`                                                      | ADR-053: this person has read the start stage's rules. Ignored outside a stage. Costs 1 rate-limit token.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
ADR-054: `contentLang` is the language of the next game's shared content: the VIP's (or the TV's)
choice, else the VIP phone's `join.lang`, else `en`.
...
`starting?: { gameId, ready: string[], countdownAt: number | null, held?: true }` (ADR-053) is the
start stage between the VIP's Start and the game, while `status` stays `selecting`: who has tapped
READY, and when the 3·2·1 began on the server clock (null while the room reads; every screen derives
the number from it). `held` is the VIP's Wait during the count.
