...
| Event   | Payload                                 | Notes                                                                                                                                                                                                                                                             |
| ------- | --------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `join`  | `{ roomCode?, name, avatarId, token? }` | `token` re-authenticates a previous player (reconnect). Without a code and with exactly one open room, joins that room.                                                                                                                                           |
| `input` | `{ seq, input }`                        | `input` is validated against the current game's `inputSchema`; `seq` is a per-client counter used to ack/dedupe.                                                                                                                                                  |
| `vip`   | `{ action, ... }`                       | `selectGame { gameId }`, `updateSettings { settings }`, `start`, `skip`, `pause`, `resume`, `end`, `kick { playerId }`, `transferVip { playerId }`, `lock`, `unlock`, `playAgain`, `toLobby`. Ignored (and counted for rate limiting) when the sender is not VIP. |
