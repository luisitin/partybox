...
| `qrcode`                      | server, web                                          | renders the join URL as an SVG QR code locally (no network, ADR-012); the web build draws its own in the browser   |
| `peerjs`                      | web                                                  | WebRTC data channels + signalling for the GitHub Pages build, which has no server to host a room (ADR-034)         |
| `vite`                        | client (build), web (build), server (dev middleware) | bundler + dev server with HMR; middleware mode keeps one port                                                      |
...
| `@types/node`, `@types/react`, `@types/react-dom`, `@types/qrcode` | root / client / web / games / server | type definitions                                                                |
| `@vitejs/plugin-react`                                             | client, web                          | JSX transform + fast refresh for Vite                                           |
