...
function matrix(ideas) {
  ...
  for (const [axis, values] of Object.entries(AXES)) {
    ...
    for (const v of values) {
      const hits = ideas
        .filter((i) => [].concat(i.meta.axes[axis]).includes(v))
        .map((i) => i.file.slice(0, 3));
...
function writeIndex(ideas, built) {
  const rowsMd = ideas.map(({ file, meta }, k) => {
    const b = built[k];
    return `| [${b.number}](${b.out}) | ${meta.title} | ${meta.status} | ${meta.players.min}–${meta.players.max} | ${meta.duration} | ${meta.difficulty} | ${meta.interaction.join('/')} | ${[].concat(meta.axes.structure).join('/')} | ${[].concat(meta.axes.skill).join('/')} | ${meta.tone.join('/')} |`;
  });
  ...
  const cards = ideas
    .map(({ meta }, k) => {
      ...
      return `<a class="card" href="${b.out}">
        ...
        <div class="tags">${[...meta.interaction, ...meta.tone].map((t) => `<span>${esc(t)}</span>`).join('')}</div>
      </a>`;
    })
    .join('\n');
