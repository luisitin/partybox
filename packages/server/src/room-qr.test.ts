...
describe('I-787: the QR belongs to the room', () => {
  it('asking for a room gives that room\'s QR; no room gives the house QR', async () => {
    await fetch(`${url}/api/rooms`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ code: 'TVBX' }) });
    const second = (await (await fetch(`${url}/api/info?room=TVBX`)).json()) as { qrUrl: string; houseRoom: string };
    expect(second.qrUrl).toMatch(/\?room=TVBX$/);
    const house = (await (await fetch(`${url}/api/info`)).json()) as { qrUrl: string; houseRoom: string };
