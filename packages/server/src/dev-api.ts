...
export function registerDevApi(app: FastifyInstance, options: DevApiOptions): void {
  ...
  app.addHook('onRequest', async (req, reply) => {
    ...
    // A request the Cloudflare tunnel (Share, public-url.ts) relays also arrives from loopback, so a
    // forwarded one is not the host PC.
    const forwarded = req.headers['cf-connecting-ip'] ?? req.headers['x-forwarded-for'];
    const loopback = ['127.0.0.1', '::1', '::ffff:127.0.0.1'].includes(req.ip) && !forwarded;
    if (
      req.url.startsWith('/api/dev/') &&
      !loopback &&
      !(req.method === 'POST' && req.url.startsWith('/api/dev/reset'))
    ) {
