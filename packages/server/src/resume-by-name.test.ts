...
describe('I-741: resume by name', () => {
  it('a borrowed phone gets the seat of a player whose phone dropped', async () => {
    ...
    await new Promise<void>((r) => {
      sam.emit('join', { name: 'Sam', avatarId: 'fox' });
      sam.once('welcome', () => r());
    });
    const priya = client();
    const first = await new Promise<WelcomePayload>((r) => {
      priya.once('welcome', r);
      priya.emit('join', { name: 'Priya', avatarId: 'owl' });
    });
  ...
  it('C: a frozen phone (still connected) hands its seat over on "take my seat"', async () => {
    const leo = client();
    const first = await new Promise<WelcomePayload>((r) => {
      leo.once('welcome', r);
      leo.emit('join', { name: 'Leo', avatarId: 'owl' });
    });
    const spare = client();
    const taken = await new Promise<ErrorPayload>((r, j) => {
      spare.once('error', r);
      spare.once('welcome', () => j(new Error('welcomed without takeOver')));
      spare.emit('join', { name: 'Leo', avatarId: 'cat' });
    });
    ...
    const back = await new Promise<WelcomePayload>((r, j) => {
      ...
      spare.once('error', (e: ErrorPayload) => j(new Error(e.code)));
      setTimeout(() => j(new Error('no welcome')), 3000);
      spare.emit('join', { name: 'Leo', avatarId: 'cat', takeOver: true });
    });
    expect(back.playerId).toBe(first.playerId);
    expect(back.token).not.toBe(first.token);
    // the frozen phone's saved login no longer resumes the seat
    const stale = client();
    const again = await new Promise<string>((r) => {
      stale.once('welcome', (w: WelcomePayload) => r(w.playerId));
      stale.once('error', (e: ErrorPayload) => r(e.code));
      stale.emit('join', { name: 'Leo', avatarId: 'owl', token: first.token });
    });
    expect(again).not.toBe(first.playerId);
  });
});
