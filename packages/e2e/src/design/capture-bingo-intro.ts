...
// Usage: tsx packages/e2e/src/design/capture-bingo-intro.ts --out <dir> [--port 42140]
...
const { values } = parseArgs({
  options: { out: { type: 'string' }, port: { type: 'string', default: '42140' } },
});
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const marks = [{ name: 'intro-line', at: Date.now(), before: 0, seconds: 5 }];
