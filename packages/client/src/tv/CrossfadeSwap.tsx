...
// DOM snapshot of the outgoing screen stays on top and fades out over `--pb-motion-fast` while the
...
/** ~`--pb-motion-fast` plus a frame of slack before the snapshot is dropped. */
const GHOST_MS = 200;

/** The live screen; hands a clone of its DOM to the parent the moment it unmounts. */
function Screen({
  className,
  onLeave,
  children,
  ...
  useLayoutEffect(
    () => () => {
      // Layout cleanup runs while this subtree is still in the document.
      const el = node.current;
      if (el && el.childElementCount > 0) onLeave(el.cloneNode(true) as HTMLElement);
    },
    [onLeave],
  );
  return (
...
export function CrossfadeSwap({ swapKey, className, children }: CrossfadeSwapProps): JSX.Element {
