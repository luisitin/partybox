...
export function createMusicEngine(): MusicEngine {
  ...
  const start = (p: MusicPlan, gen: number, first = false): void => {
    ...
    void el
      .play()
      .then(() => {
        // A plan that changed while play() was pending is fading this one out: its fade-in must
        // not take the element back (rampTo replaces the running ramp, so the fade-out's stop never
        // came and the lobby's track played on under the game's at the lobby's level — the owner,
        // 2026-09-23: "it is a bit loud in blanks").
        if (gen !== generation || retired(el)) return;
  ...
  return {
    play(next) {
      if ((next?.id ?? null) === (plan?.id ?? null)) {
        // S-004 C: the same plan at a new level (the phone's soft / normal / loud) re-levels the
        // playing track instead of restarting it.
        if (next && plan && next.volume !== plan.volume) {
          plan = next;
          if (audio && !audio.paused) rampTo(audio, levelOf(next, last), 400);
        }
        return;
      }
