...
export function BigScreenHint(): JSX.Element | null {
  ...
  const [show, setShow] = useState(
    () =>
      looksLikeBigScreen() &&
      get('session', DISMISSED) === null &&
      get('local', CHOICE) !== 'player',
  );
    ...
    return (
      <div className={`${styles.card} ${styles.floating}`} role="status">
        <p className={styles.cardTitle}>
          {L('📺 Opening the TV view… {n}', { n: Math.max(countdown, 0) })}
        </p>
        <p className={styles.cardText}>{L('This screen was the TV last time.')}</p>
  ...
  return (
    <div
      ...
      role="region"
      aria-label={L('Is this the TV?')}
    >
      <p className={styles.cardTitle}>{L('📺 Is this the TV?')}</p>
...
export function PhoneOnTvHint({ code }: { code: string | null }): JSX.Element | null {
  ...
  return (
    <div className={styles.cover} style={{ zoom: unzoom }}>
      <div className={styles.card} role="region" aria-label={L('This is the TV page')}>
        <p className={styles.cardTitle}>{L('📺 This is the TV page')}</p>
        <p className={styles.cardText}>
          {L(
        ...
        <div className={styles.cardActions}>
          ...
          <button type="button" className={styles.secondary} onClick={keep}>
            {L('Keep the TV page on this phone')}
          </button>
        </div>
      </div>
    </div>
