...
export function ControllerApp(): JSX.Element {
  ...
  if (!state.joined || !state.room || !me) {
    ...
    switch (state.room.status) {
      case 'lobby':
        screen = <Lobby controller={controller} room={state.room} me={me} onSetup={() => setSetupOpen((n) => n + 1)} />;
        break;
      case 'selecting':
  ...
  return (
    <ServerClockProvider offsetMs={state.offsetMs}>
      <AvatarPhotos players={state.room?.players}>
        <ControllerShell controller={controller} state={state} me={me} audio={audio} openTheme={setupOpen}>
          <CrossfadeSwap
