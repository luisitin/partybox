...
const game = {
  ...
  settings: [
    ...
    { key: 'decks', type: 'select', label: 'Decks', default: 'wild', options: [{ value: 'wild', label: 'Wild' }, { value: 'mild', label: 'Mild' }] },
  ],
} as unknown as GameSummary;
const [rounds, timed, decks] = game.settings as [GameSummary['settings'][number], GameSummary['settings'][number], GameSummary['settings'][number]];
