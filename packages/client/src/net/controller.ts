...
export function createController(url?: string): Controller {
  ...
  const channel = typeof BroadcastChannel === 'undefined' ? null : new BroadcastChannel('partybox:seat');
  channel?.addEventListener('message', (e: MessageEvent<{ type: string; token?: string }>) => {
    const mine = loadSession()?.token;
    if (e.data.type === 'who' && store.get().joined && !holdReconnect && mine && e.data.token === mine)
      channel.postMessage({ type: 'here' });
  });
  const askOtherTabs = (token: string): Promise<boolean> =>
    new Promise((resolve) => {
