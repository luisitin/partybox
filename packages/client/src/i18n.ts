...
type JoinStrings = Pick<
  JoinAll,
  'title' | 'name' | 'namePlaceholder' | 'avatar' | 'usePhoto' | 'useFace' | 'joiningRoom' | 'submit' | 'needName' | 'joining'
>;
const JOIN_L10N: Record<Exclude<JoinLang, 'en'>, JoinStrings> = {
  es: { title: 'Únete a la fiesta', name: 'Tu nombre', namePlaceholder: 'p. ej. Sam', avatar: 'Elige un avatar', usePhoto: 'Usar una foto', useFace: 'Usar una cara', joiningRoom: 'Entrando en la sala', submit: 'Entrar', needName: 'Escribe un nombre para entrar', joining: 'Entrando…' },
  de: { title: 'Mach mit', name: 'Dein Name', namePlaceholder: 'z. B. Sam', avatar: 'Wähl einen Avatar', usePhoto: 'Foto verwenden', useFace: 'Lieber ein Gesicht', joiningRoom: 'Raum', submit: 'Beitreten', needName: 'Gib einen Namen ein', joining: 'Tritt bei…' },
  fr: { title: 'Rejoins la fête', name: 'Ton prénom', namePlaceholder: 'p. ex. Sam', avatar: 'Choisis un avatar', usePhoto: 'Utiliser une photo', useFace: 'Plutôt un visage', joiningRoom: 'Salle', submit: 'Rejoindre', needName: 'Entre un prénom pour rejoindre', joining: 'Connexion…' },
  pt: { title: 'Entra na festa', name: 'O teu nome', namePlaceholder: 'p. ex. Sam', avatar: 'Escolhe um avatar', usePhoto: 'Usar uma foto', useFace: 'Usar uma cara', joiningRoom: 'A entrar na sala', submit: 'Entrar', needName: 'Escreve um nome para entrar', joining: 'A entrar…' },
};
...
export function lobbyStrings(lang: JoinLang = joinLang()): Omit<typeof t.lobby, 'waitingForVip' | 'addBot'> & { waitingForVip: string; addBot: string } {
