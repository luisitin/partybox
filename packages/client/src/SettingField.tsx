...
export function SettingField({
  ...
  const implied = spec.impliedBy !== undefined && settings?.[spec.impliedBy.key] === spec.impliedBy.value;
  switch (spec.type) {
    case 'boolean':
      return (
        <label className={styles.setting} htmlFor={id} style={implied ? { opacity: 0.5 } : undefined}>
          <span className={styles.settingLabel}>
            {spec.label}
            {implied && spec.impliedBy ? <small>{spec.impliedBy.note}</small> : spec.description ? <small>{spec.description}</small> : null}
          </span>
          <input
            id={id}
            type="checkbox"
