...
function play(room: RoomState, at: number, tapper?: string): RoomState {
  ...
  if (tapper)
    r = applyRoomEvent(
      r,
      { type: 'input', now: at, playerId: tapper, input: { type: 'tap' } },
      deps,
    ).room;
...
describe('I-652: tonight', () => {
  it('each finished game is added with its winner; a scoreless one has no winner', () => {
    ...
    expect(first.tonight).toEqual([
      expect.objectContaining({
        gameId: 'fake',
        winners: [expect.objectContaining({ name: 'P1' })],
        botsWon: false,
      }),
    ]);
      ...
      game: {
        ...
        state: {
          ...
          players: {
            ...
            p1: { ...room.game!.state.players.p1!, bot: true },
          },
        },
      },
    };
  ...
  it('a gap of more than 3 hours starts a new night; at most 6 games are kept', () => {
    ...
    for (let i = 0; i < 8; i++)
      tonight = play({ ...playingRoom(2), tonight }, T0 + 200 + i).tonight;
