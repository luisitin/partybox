...
describe('I-347: the VIP comes back', () => {
  it('in the lobby the role does not pass on', () => {
    let room = applyRoomEvent(
      lobby(),
      { type: 'disconnect', now: T0 + 1000, playerId: 'p1' },
      deps,
    ).room;
  ...
  it('the former VIP can take the role back; nobody else can', () => {
    ...
    room = {
      ...
      players: {
        ...room.players,
        p1: { ...room.players['p1']!, isVip: false },
        p2: { ...room.players['p2']!, isVip: true },
      },
    };
    const other = applyRoomEvent(
      room,
      { type: 'vip', now: T0 + 60_000, playerId: 'p2', action: { action: 'reclaimVip' } },
      deps,
    ).room;
    expect(other.vipId).toBe('p2');
    const back = applyRoomEvent(
      room,
      { type: 'vip', now: T0 + 60_000, playerId: 'p1', action: { action: 'reclaimVip' } },
      deps,
    ).room;
