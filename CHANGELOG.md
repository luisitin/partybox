...
- **One rulebook for every agent, and an onboarding guide for Codex** (ADR-055). Each folder's rules moved from `CLAUDE.md` to `AGENTS.md`: Codex reads that name, and Claude Code imports it through a 3-line `CLAUDE.md`. `pnpm check-drift` keeps the two from drifting. `docs/CODEX_GUIDE.md` gives an agent new to this machine the setup, ports, hub workflow, the owner's preferences and the gotchas that lived only in Claude session notes.
